# Load Larger LSTM network and generate text char by char using seed
import sys
import argparse
import numpy
...
if __name__ == '__main__':
	...
	#list_of_methods = ['wordnet', 'lesk', 'custom_lesk', 'combo']
	...
	ap.add_argument("-d", "--data", required=True, help="Training data corpus used to train on.")
	ap.add_argument("-nw", "--network-weights", required=True, help="Network weights of the RNN to sample from.")

	args = vars(ap.parse_args())
	nw_filename = args["network_weights"]
	data_filename = args["data"] # only for testing purposes

	print("[SETUP] Network weights file of the RNN: " + str(nw_filename))
	...
	#filename = "weights-herons-improvement-20-1.6989-bigger.hdf5"
	model.load_weights(nw_filename)
