...
class HaikuGeneratorLSTM:
	...
	def train_word_lvl(self, embedding, train=True):
		...
		if(train):
			...
			# fit the model
			model.fit_generator(self.TextDataGenerator(word_phrase_pairs, len_longest_phrase, n_unique_words, word_to_index, embedding), steps_per_epoch=1000, epochs=150, verbose=1,callbacks=callbacks_list)
